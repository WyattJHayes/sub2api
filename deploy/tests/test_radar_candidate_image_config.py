...
class RadarCandidateImageConfigTest(unittest.TestCase):
    ...
    def _render(self, *, include_reliability: bool = False) -> dict:
        ...
        with tempfile.TemporaryDirectory(prefix="radar-compose-config-") as temp_dir:
            ...
            secret_files = {
                "RADAR_DATABASE_PASSWORD_FILE": "database-password",
                "RADAR_POSTGRES_PASSWORD_FILE": "postgres-password",
                "RADAR_DATABASE_PGPASS_FILE": "database.pgpass",
            }
            for variable, filename in secret_files.items():
                secret_path = secret_dir / filename
                secret_path.write_text("synthetic-test-value\n", encoding="utf-8")
                secret_path.chmod(0o600)
                env[variable] = str(secret_path)

...
IMAGE_TAG_RE = re.compile(r"^0\.2\.4-radar-v24-(\d{8}T\d{6}Z)$")
...
def validate_inputs(inputs: BuildInputs) -> None:
    ...
    if image_tag_match is None:
        raise ValueError("image_tag must use 0.2.4-radar-v24-YYYYMMDDTHHMMSSZ format")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build and record immutable Sub2API v0.2.4 Radar private GHCR candidates."
    )

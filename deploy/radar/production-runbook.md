...
python3 deploy/radar/production_authorization_audit.py \
  ...
  --candidate-image-record "$EVIDENCE_DIR/candidate-image-record.json" \
  --pre-promotion-snapshot "$EVIDENCE_DIR/pre-promotion-snapshot.json" \
  --preflight "$EVIDENCE_DIR/preflight.json" \
  --authorization-claim "$EVIDENCE_DIR/authorization-claim.json" \
  --max-age-seconds 3600 \
  --output "$EVIDENCE_DIR/authorization.json"
```

The release CLI accepts only private evidence paths. Candidate digests, source SHA, target identity and predecessor hashes are derived from those files. The output is an envelope written with mode `0600`; any cross-release, cross-target, stale, or changed predecessor blocks the command.
...
python3 deploy/radar/production_backup_audit.py \
  --release-id "$RELEASE_ID" \
  --candidate-image-record "$EVIDENCE_DIR/candidate-image-record.json" \
  --authorization "$EVIDENCE_DIR/authorization.json" \
  --backup-path "$BACKUP_PATH" \
  --restore-envelope "$EVIDENCE_DIR/isolation-migration.json" \
  --deployment-dir /opt/sub2api \
  --max-age-seconds 3600 \
  --min-size-bytes 1024 \
  --expected-schema-migrations 286 \
  --output "$EVIDENCE_DIR/backup.json"
```

The backup CLI reopens the actual file with `O_NOFOLLOW`, validates regular-file type and mode `0600`, checks canonical path/device/inode/size/mtime/SHA256, and binds the isolated restore envelope with migration count `286`. A changed file or symlink fails closed.
...
python3 deploy/radar/production_promotion_manifest.py \
  --release-id "$RELEASE_ID" \
  --candidate-image-record "$EVIDENCE_DIR/candidate-image-record.json" \
  --isolation "$EVIDENCE_DIR/isolation.json" \
  --migration-closure "$EVIDENCE_DIR/isolation-migration.json" \
  --browser-closure "$EVIDENCE_DIR/isolation-browser.json" \
  --preflight "$EVIDENCE_DIR/preflight.json" \
  --target-snapshot "$EVIDENCE_DIR/pre-promotion-snapshot.json" \
  --authorization "$EVIDENCE_DIR/authorization.json" \
  --backup "$EVIDENCE_DIR/backup.json" \
  --output "$EVIDENCE_DIR/promotion-manifest.json"
```
...
The manifest contains only evidence paths and recomputed hashes. The audit derives candidate and rollback repository digests from the candidate record and target snapshot and exits nonzero on any missing, stale, cross-target, cross-release, or tampered input. No digest or pass/fail claim is accepted as a command-line scalar.
...
python3 deploy/radar/production_rollback_audit.py \
  --rollback-evidence "$EVIDENCE_DIR/rollback.json" \
  --expected-schema-migrations 286 \
  --output "$EVIDENCE_DIR/rollback-audit.json"
```
...
python3 deploy/radar/production_release_closure_evidence.py \
  ...
  --candidate-image-record "$EVIDENCE_DIR/candidate-image-record.json" \
  --isolation "$EVIDENCE_DIR/isolation.json" \
  --promotion "$EVIDENCE_DIR/promotion.json" \
  --smoke "$EVIDENCE_DIR/smoke.json" \
  --rollback "$EVIDENCE_DIR/rollback.json" \
  --restoration "$EVIDENCE_DIR/restoration.json" \
  --output "$EVIDENCE_DIR/closure.json"
```

The builder records only references to validated private envelopes. Missing, copied, or modified evidence fails closed. Then run the final audit:

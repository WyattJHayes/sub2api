...
APP_VERSION = "0.1.173"
SCHEMA_VERSION = "radar-v01173-image-record-v1"
...
IMAGE_TAG_RE = re.compile(r"^0\.1\.173-radar-v13-(\d{8}T\d{6}Z)$")
...
def validate_inputs(inputs: BuildInputs) -> None:
    ...
    if inputs.commit != inputs.source_sha256[:40]:
        raise ValueError("commit must equal source_sha256 prefix")
    ...
    if image_tag_match is None:
        raise ValueError("image_tag must use 0.1.173-radar-v13-YYYYMMDDTHHMMSSZ format")
...
def verify_image_provenance(
    ...
    if inspection.get("Id") != expected_config_digest:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build and record immutable Sub2API v0.1.173 Radar private GHCR candidates."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="radar-v01173-build-") as directory:
            ...
            worker_version = verify_worker(
                WORKER_REPOSITORY, worker_manifest, inputs.version
            )

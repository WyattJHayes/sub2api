...
APP_VERSION = "0.1.182"
SCHEMA_VERSION = "radar-v01182-image-record-v1"
...
IMAGE_TAG_RE = re.compile(r"^0\.1\.182-radar-v18-(\d{8}T\d{6}Z)$")
...
def validate_inputs(inputs: BuildInputs) -> None:
    ...
    if image_tag_match is None:
        raise ValueError("image_tag must use 0.1.182-radar-v18-YYYYMMDDTHHMMSSZ format")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build and record immutable Sub2API v0.1.182 Radar private GHCR candidates."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="radar-v01182-build-") as directory:

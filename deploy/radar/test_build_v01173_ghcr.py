...
build_tool = load_script("radar_build_v01173_ghcr", "build_v01173_ghcr.py")
...
def valid_inputs() -> Any:
    return build_tool.BuildInputs(
        version="0.1.173",
        image_tag="0.1.173-radar-v13-20260809T010203Z",
        commit="c" * 40,
        source_sha256="c" * 64,
...
class BuildV01173GhcrTests(unittest.TestCase):
    def test_constructs_exact_control_plane_and_worker_commands(self) -> None:
        ...
        self.assertIn("VERSION=0.1.173", control)
    ...
    def test_rejects_source_sha256_that_does_not_match_the_current_source_tree(self) -> None:
        ...
        with patch.object(build_tool, "source_tree_sha256", return_value="c" * 64):
            build_tool.validate_current_source(valid_inputs())
    ...
    def test_source_sha256_is_required_and_must_match_revision_prefix(self) -> None:
        command = [
            "--version", "0.1.173",
            "--image-tag", "0.1.173-radar-v13-20260809T010203Z",
            "--commit", "c" * 40,
            "--date", "2026-08-09T01:02:03Z",
    ...
    def test_requires_exact_app_version_and_timestamped_v13_image_tag(self) -> None:
        cases = (
            (replace(valid_inputs(), version="0.1.173-radar-v13"), "version must equal 0.1.173"),
            (replace(valid_inputs(), image_tag="0.1.173"), "image_tag must use"),
            (
                replace(valid_inputs(), image_tag="0.1.173-radar-v13-20260809T010204Z"),
    ...
    def test_candidate_build_runbook_requires_source_sha_and_correct_tag_timestamp(self) -> None:
        runbook = (RADAR_DIR / "v01173-upgrade-runbook.md").read_text(encoding="utf-8")
    ...
    def test_writes_redacted_mode_0600_dual_image_record(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual("radar-v01173-image-record-v1", stored["schema_version"])
            self.assertEqual("0.1.173", stored["version"])
    ...
    def test_historical_record_uses_its_release_version_after_worker_source_advances(self) -> None:
        inputs = valid_inputs()
        with (RADAR_DIR.parents[1] / "radar-worker" / "pyproject.toml").open("rb") as stream:
            current_worker_version = tomllib.load(stream)["project"]["version"]
        self.assertNotEqual(inputs.version, current_worker_version)
        record = build_tool.build_record(
            inputs,
            control_manifest=SHA_A,
            control_config=SHA_B,
            ...
            worker_manifest=SHA_B,
            worker_config=SHA_A,
            worker_version=inputs.version,
        )
        self.assertEqual(inputs.version, record["worker"]["version_output"])

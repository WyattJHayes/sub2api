...
The follow-up verification also passed the focused outbox repository integration suite, outbox dispatcher and runtime unit suite, runtime cancellation and fencing cases, the outbox runtime race suite, server and route tests, Radar worker Ruff and Mypy checks, 151 Radar worker Pytest cases, both staging Compose config checks with required variables, and the Linux embed artifact SHA/version smoke test. The race suite required the host Go cache and local test listeners, so it was rerun in the controlled full-permission environment.

...
Current status: staging evidence, disposable migration proof, production configuration hashes, a fail-closed production target preflight, a fail-closed promotion input audit tool, and a current failed promotion audit run are recorded. The production promotion input audit cannot pass yet because `/opt/sub2api` has no running production Compose project, no active production application container, no current production logical backup, no verified active production image digest, and `.env` is still mode `644`.

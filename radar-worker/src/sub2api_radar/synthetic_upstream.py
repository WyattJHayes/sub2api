...
MODEL_FAILURES = {
    "radar-synthetic-baseline": frozenset(),
    "radar-synthetic-healthy": frozenset(),
    "radar-synthetic-watered": frozenset({"model_fingerprint", "structure_compliance"}),
    "radar-synthetic-degraded": frozenset({"knowledge_freshness", "instruction_hierarchy"}),
    "radar-synthetic-candidate": QUALITY_DIMENSIONS,
}


def scenario_output(model: str, messages: object) -> str:
    if model not in MODEL_FAILURES or not isinstance(messages, list) or not messages:
        raise ValueError("unsupported synthetic quality request")
    content = messages[0].get("content") if isinstance(messages[0], dict) else None
    if not isinstance(content, str) or not content.startswith("RADAR_QUALITY_DIMENSION="):
        raise ValueError("synthetic quality dimension marker is required")
    dimension = content.splitlines()[0].partition("=")[2]
    if dimension not in QUALITY_DIMENSIONS:
        raise ValueError("unsupported synthetic quality dimension")
    return "Lyon" if dimension in MODEL_FAILURES[model] else "Paris"
...
class SyntheticUpstreamHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            output = scenario_output(model, messages)
        except ValueError:
            self._send(
                HTTPStatus.BAD_REQUEST,
                {"error": {"code": "invalid_quality_fixture"}},
            )
            return

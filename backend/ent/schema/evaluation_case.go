...
func (EvaluationCase) Fields() []ent.Field {
	return []ent.Field{
		...
		field.JSON("prompt_spec", map[string]any{}).SchemaType(map[string]string{dialect.Postgres: "jsonb"}).Optional().Immutable(),
		field.JSON("expected_spec", map[string]any{}).SchemaType(map[string]string{dialect.Postgres: "jsonb"}).Optional().Immutable(),
		field.String("encrypted_spec").SchemaType(map[string]string{dialect.Postgres: "text"}).Optional().Nillable().Immutable(),
		field.JSON("execution_spec", map[string]any{}).SchemaType(map[string]string{dialect.Postgres: "jsonb"}).Immutable(),
		field.String("quality_dimension").SchemaType(map[string]string{dialect.Postgres: "text"}).Optional().Immutable(),
		field.JSON("quality_probe_spec", map[string]any{}).SchemaType(map[string]string{dialect.Postgres: "jsonb"}).Default(map[string]any{}).Immutable(),

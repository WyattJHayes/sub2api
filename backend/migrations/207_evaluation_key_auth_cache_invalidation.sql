-- is_evaluation participates in API-key authentication. A governance enablement
-- changes that field through direct SQL, so the durable cache invalidation
-- trigger must evict snapshots created while the key was ordinary.
...
BEGIN
    ...
    IF OLD.key IS DISTINCT FROM NEW.key
       ...
       OR OLD.is_evaluation IS DISTINCT FROM NEW.is_evaluation THEN
        PERFORM enqueue_auth_cache_invalidation(OLD.key);

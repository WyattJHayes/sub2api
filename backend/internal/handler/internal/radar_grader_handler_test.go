...
type radarGraderHandlerRepoStub struct {
	...
	analysisSubmission       service.AggregateSubmission
	analysisErr              error
	identifyErr              error
	claimErr                 error
	submitErr                error
...
func (s *radarGraderHandlerRepoStub) CompleteAnalysisJob(_ context.Context, _ uuid.UUID, _ string, submission service.AggregateSubmission, _ ...int64) (*service.AggregateSnapshot, error) {
	s.analysisSubmission = submission
...
func TestRadarGraderHandlerMapsDependencyDeadlineToRetryableUnavailable(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &radarGraderHandlerRepoStub{
		workerID: uuid.New(),
		claimErr: fmt.Errorf("redis claim: %w", context.DeadlineExceeded),
	}
	h := NewRadarGraderHandler(repo, &config.Config{})
	r := gin.New()
	r.POST("/internal/radar/v1/grading-leases:claim", h.ClaimGradingLease)
	...
	require.NotContains(t, resp.Body.String(), "redis")
	require.NotContains(t, resp.Body.String(), "context deadline")
}
...
func TestRadarGraderHandlerSignsGatewayEvaluationTokenForAssignment(t *testing.T) {
	...
	repo := &radarGraderHandlerRepoStub{
		...
		assignmentLease: &service.AssignmentLease{
			...
			ModelConfig: json.RawMessage(`{"model":"gateway-model"}`),
...
func TestRadarGraderHandlerRejectsRunnerTokenForQualityAggregate(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &radarGraderHandlerRepoStub{identifyErr: service.ErrWorkerKindMismatch}
	h := NewRadarGraderHandler(repo, &config.Config{})
	r := gin.New()
	r.POST("/internal/radar/v1/analysis-jobs/:id/complete", h.CompleteAnalysisJob)
	req := httptest.NewRequest(http.MethodPost,
		"/internal/radar/v1/analysis-jobs/"+uuid.NewString()+"/complete",
		bytes.NewBufferString(`{"lease_token":"analysis-lease-token","aggregate":{},"quality_report":{}}`))
	req.Header.Set("Authorization", "Bearer runner-token")
	resp := httptest.NewRecorder()
	...
	require.Equal(t, http.StatusForbidden, resp.Code)
	require.Nil(t, repo.analysisSubmission.QualityReport)
}

func TestRadarGraderHandlerPassesQualityReportIntoAggregateSubmission(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &radarGraderHandlerRepoStub{workerID: uuid.New()}
	h := NewRadarGraderHandler(repo, &config.Config{})
	r := gin.New()
	...
	require.Equal(t, http.StatusOK, resp.Code)
	require.NotNil(t, repo.analysisSubmission.QualityReport)
}

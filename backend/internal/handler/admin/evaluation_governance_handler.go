...
var radarTrackedModelAliasPattern = regexp.MustCompile(`^[A-Za-z0-9._:/-]{1,128}$`)

func radarProjectionCollection(items any) any {
...
func (h *RadarGovernanceHandler) RegisterTrackedModel(c *gin.Context) {
	...
	if !radarTrackedModelAliasPattern.MatchString(req.ModelAlias) {
		response.BadRequest(c, "Invalid model alias")
		return
	}
	repo, ok := h.repo.(service.RadarTrackedModelRepository)
	if !ok || repo == nil {
		response.Error(c, http.StatusServiceUnavailable, "Radar model inventory is not available")
		return
	}
	model, err := repo.RegisterTrackedModel(c.Request.Context(), req.ModelAlias, actorID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *RadarGovernanceHandler) UntrackModel(c *gin.Context) {
	actorID, ok := h.require(c, service.PermissionDatasetManage)
	if !ok {
		return
	}
	modelAlias := strings.TrimSpace(c.Param("alias"))
	if !radarTrackedModelAliasPattern.MatchString(modelAlias) {
		response.BadRequest(c, "Invalid model alias")
		return
	}
	repo, ok := h.repo.(service.RadarTrackedModelRepository)
	...
	if err := repo.UntrackModel(c.Request.Context(), modelAlias, actorID); err != nil {
		if errors.Is(err, service.ErrTrackedModelNotFound) {
			response.NotFound(c, "Tracked model not found")
			return
		}
		response.ErrorFrom(c, err)
		return
	}
	response.Success(c, gin.H{"model_alias": modelAlias, "status": "untracked"})
}
...
func (h *RadarGovernanceHandler) Overview(c *gin.Context) {
	...
	response.Success(c, gin.H{"freshness": time.Now().UTC(), "models": radarProjectionCollection(models), "alerts": radarProjectionCollection(alerts), "gates": radarProjectionCollection(gates), "workers": radarProjectionCollection(workers), "summary": gin.H{"models": len(models), "open_alerts": openAlerts, "blocked_gates": blockedGates, "healthy_workers": healthyWorkers}})
}
...
func (h *RadarGovernanceHandler) projectionList(c *gin.Context, load func(service.RadarProjectionRepository, context.Context) (any, error)) {
	...
	response.Success(c, radarProjectionCollection(items))
}

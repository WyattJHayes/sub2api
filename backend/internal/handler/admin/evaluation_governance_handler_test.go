...
type radarGovernanceHandlerRepoStub struct {
	*service.StaticRadarAuthorizer
	...
	trackedModel              *service.RadarTrackedModel
	untrackedModelAlias       string
	untrackErr                error
}

type radarRunControlHandlerRepoStub struct{ radarGovernanceHandlerRepoStub }

type radarProjectionHandlerRepoStub struct {
	*radarGovernanceHandlerRepoStub
	models   []service.RadarModelHealthProjection
	runs     []service.RadarRunProjection
	alerts   []service.RadarAlertProjection
	gates    []service.RadarGateProjection
	workers  []service.RadarWorkerProjection
	datasets []service.RadarDatasetProjection
}

func (s *radarProjectionHandlerRepoStub) ListModelHealth(context.Context) ([]service.RadarModelHealthProjection, error) {
	return s.models, nil
}

func (s *radarProjectionHandlerRepoStub) ListRuns(context.Context) ([]service.RadarRunProjection, error) {
	return s.runs, nil
}

func (s *radarProjectionHandlerRepoStub) ListAlerts(context.Context) ([]service.RadarAlertProjection, error) {
	return s.alerts, nil
}

func (s *radarProjectionHandlerRepoStub) ListGates(context.Context) ([]service.RadarGateProjection, error) {
	return s.gates, nil
}

func (s *radarProjectionHandlerRepoStub) ListWorkers(context.Context) ([]service.RadarWorkerProjection, error) {
	return s.workers, nil
}

func (s *radarProjectionHandlerRepoStub) ListDatasets(context.Context) ([]service.RadarDatasetProjection, error) {
...
func (s *radarGovernanceHandlerRepoStub) RegisterTrackedModel(_ context.Context, modelAlias string, actorID int64) (*service.RadarTrackedModel, error) {
	s.trackedModel = &service.RadarTrackedModel{ModelAlias: modelAlias, CreatedBy: actorID, CreatedAt: time.Now().UTC()}
	return s.trackedModel, nil
}

func (s *radarGovernanceHandlerRepoStub) UntrackModel(_ context.Context, modelAlias string, _ int64) error {
	s.untrackedModelAlias = modelAlias
	return s.untrackErr
}
...
func TestRadarGovernanceHandlerSerializesEmptyProjectionAsArray(t *testing.T) {
	auth := service.NewStaticRadarAuthorizer(map[int64][]service.RadarRole{77: {service.RoleViewer}})
	repo := &radarProjectionHandlerRepoStub{
	...
	NewRadarGovernanceHandler(repo).EmptyAlerts(c)
...
func TestRadarGovernanceHandlerOverviewSerializesEmptyCollectionsAsArrays(t *testing.T) {
	auth := service.NewStaticRadarAuthorizer(map[int64][]service.RadarRole{77: {service.RoleViewer}})
	repo := &radarProjectionHandlerRepoStub{
		radarGovernanceHandlerRepoStub: &radarGovernanceHandlerRepoStub{StaticRadarAuthorizer: auth},
	}
	...
	NewRadarGovernanceHandler(repo).Overview(c)
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &body))
	for _, collection := range []json.RawMessage{body.Data.Models, body.Data.Alerts, body.Data.Gates, body.Data.Workers} {
		require.JSONEq(t, `[]`, string(collection))
	}
}
...
func TestRadarGovernanceHandlerRegistersTrackedModelWithAuthenticatedActor(t *testing.T) {
	auth := service.NewStaticRadarAuthorizer(map[int64][]service.RadarRole{77: {service.RoleQualityAdmin}})
	repo := &radarGovernanceHandlerRepoStub{StaticRadarAuthorizer: auth}
	h := NewRadarGovernanceHandler(repo)
	c := radarGovernanceTestContext(77)
	c.Request = httptest.NewRequest(http.MethodPost, "/", bytes.NewBufferString(`{"model_alias":"gpt-5.6-sol"}`))

	h.RegisterTrackedModel(c)

	require.Equal(t, http.StatusCreated, c.Writer.Status())
	require.NotNil(t, repo.trackedModel)
	require.Equal(t, "gpt-5.6-sol", repo.trackedModel.ModelAlias)
	require.Equal(t, int64(77), repo.trackedModel.CreatedBy)
}

func TestRadarGovernanceHandlerUntracksModelForAuthenticatedTenant(t *testing.T) {
	auth := service.NewStaticRadarAuthorizer(map[int64][]service.RadarRole{77: {service.RoleQualityAdmin}})
	repo := &radarGovernanceHandlerRepoStub{StaticRadarAuthorizer: auth}
	h := NewRadarGovernanceHandler(repo)
	recorder := httptest.NewRecorder()
	...
	c.Params = gin.Params{{Key: "alias", Value: "gpt-5.6-sol"}}

	h.UntrackModel(c)

	require.Equal(t, http.StatusOK, recorder.Code)
	require.Equal(t, "gpt-5.6-sol", repo.untrackedModelAlias)
	var body struct {
		Data struct {
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &body))
	require.Equal(t, "gpt-5.6-sol", body.Data.ModelAlias)
	require.Equal(t, "untracked", body.Data.Status)
}

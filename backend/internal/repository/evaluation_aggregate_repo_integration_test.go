...
func TestCellRevisionPersistsQualityReportWithAggregateSnapshot(t *testing.T) {
	ctx := context.Background()
	fixture, gradingRepo, runID := prepareAggregateCellFixture(t)
	_, err := integrationDB.ExecContext(ctx, `
		INSERT INTO quality_policy_versions (id, tenant_id, version, policy, created_by)
		VALUES ($1,$2,'quality-v1',$3::jsonb,$2)`,
		uuid.New(), fixture.userID, service.DefaultQualityPolicyJSON())
	require.NoError(t, err)
	...
	require.NoError(t, integrationDB.QueryRowContext(ctx, `
		...
		WHERE tenant_id=$1 AND run_id=$2 AND model_alias='route-a'`, fixture.userID, runID).Scan(&reports))
	require.Equal(t, 1, reports)
}

func TestCellRevisionPublishesQualityReportForEachAggregateRevision(t *testing.T) {
	ctx := context.Background()
	fixture, gradingRepo, runID := prepareAggregateCellFixture(t)
	_, err := integrationDB.ExecContext(ctx, `
		INSERT INTO quality_policy_versions (id, tenant_id, version, policy, created_by)
	...
	require.NoError(t, err)
	firstReport := validQualityReportForPersistence()
	firstReport.RunID = runID
	firstReport.ModelAlias = "route-a"
	firstSnapshot, err := gradingRepo.CompleteAnalysisJob(ctx, firstLease.ID, firstLease.Token, service.AggregateSubmission{
	...
	require.NoError(t, err)
	require.Greater(t, secondJob.AggregateRevision, firstJob.AggregateRevision)
	configureAggregateStatisticsWorker(t, fixture.workerIDs[0])
	secondLease, err := gradingRepo.ClaimAnalysisJob(ctx, fixture.workerIDs[0], []string{"coding"}, time.Minute)
	require.NoError(t, err)
	secondReport := validQualityReportForPersistence()
	secondReport.RunID = runID
	secondReport.ModelAlias = "route-a"
	secondReport.Dimensions[0].ReferenceBaselineDeltaPP = repositoryQualityFloatPointer(5)
	secondReport.Dimensions[0].Status = service.QualityConclusionObserve
	secondReport.Dimensions[0].EvidenceCode = service.QualityEvidenceWithinPolicyBounds
	secondReport.DegradationRisk = service.QualityConclusionObserve
	secondReport.OverallConclusion = service.QualityConclusionObserve
	secondSnapshot, err := gradingRepo.CompleteAnalysisJob(ctx, secondLease.ID, secondLease.Token, service.AggregateSubmission{
	...
	require.NoError(t, err)
	require.NotEqual(t, firstSnapshot.ID, secondSnapshot.ID)

	var reports, snapshots int
	require.NoError(t, integrationDB.QueryRowContext(ctx, `
		SELECT COUNT(*) FROM quality_reports
		WHERE tenant_id=$1 AND run_id=$2 AND model_alias='route-a'`, fixture.userID, runID).Scan(&reports))
	require.NoError(t, integrationDB.QueryRowContext(ctx, `
		SELECT COUNT(*) FROM evaluation_aggregate_snapshots WHERE run_id=$1`, runID).Scan(&snapshots))
	require.Equal(t, 2, reports)
	require.Equal(t, 2, snapshots)

	latest, err := NewModelQualityRepository(integrationDB).GetPublicQualityReport(
		service.WithRadarTenant(ctx, fixture.userID), "route-a",
	)
	require.NoError(t, err)
	require.Equal(t, service.QualityConclusionObserve, latest.OverallConclusion)
}

func TestCellRevisionRejectsForgedQualityReportAtomically(t *testing.T) {
	tests := []struct {
		name   string
		mutate func(*service.QualityReportPublication)
	}{
		{
			name: "high risk dimension status",
			mutate: func(report *service.QualityReportPublication) {
				report.Dimensions[0].Status = service.QualityConclusionHighRisk
			},
		},
		{
			name: "fingerprint evidence code",
			mutate: func(report *service.QualityReportPublication) {
				report.Dimensions[1].EvidenceCode = service.QualityEvidenceFingerprintMismatch
			},
		},
		{
			name: "aggregate conclusion",
			mutate: func(report *service.QualityReportPublication) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			var reports, snapshots int
			require.NoError(t, integrationDB.QueryRowContext(ctx, `
				SELECT COUNT(*) FROM quality_reports WHERE tenant_id=$1 AND run_id=$2`, fixture.userID, runID).Scan(&reports))
			require.NoError(t, integrationDB.QueryRowContext(ctx, `
				SELECT COUNT(*) FROM evaluation_aggregate_snapshots WHERE run_id=$1`, runID).Scan(&snapshots))
...
func TestCellRevisionRejectsCompletionByDifferentBoundWorker(t *testing.T) {
	ctx := context.Background()
	fixture, gradingRepo, runID := prepareAggregateCellFixture(t)
	aggregateRepo := NewEvaluationAggregateRepository(integrationDB)
	job, err := aggregateRepo.EnsureCellAnalysisJob(ctx, service.CellAnalysisJobRequest{
	...
	require.NoError(t, err)
	configureAggregateStatisticsWorker(t, fixture.workerIDs[0])
	configureAggregateStatisticsWorker(t, fixture.workerIDs[1])
	lease, err := gradingRepo.ClaimAnalysisJob(ctx, fixture.workerIDs[0], []string{"coding"}, time.Minute)
	require.NoError(t, err)
	require.NotNil(t, lease)

	_, err = gradingRepo.CompleteAnalysisJob(
		service.WithRadarWorkerID(ctx, fixture.workerIDs[1]),
		lease.ID,
		lease.Token,
	...
	var status string
	require.NoError(t, integrationDB.QueryRowContext(ctx, `SELECT status FROM evaluation_analysis_jobs WHERE id=$1`, lease.ID).Scan(&status))
	require.Equal(t, "leased", status)
}
...
func TestMultiCellRunRequiresExplicitGlobalSnapshot(t *testing.T) {
	...
	require.Nil(t, lease.QualityContext)

...
func (r *radarGovernanceRepository) controlRun(ctx context.Context, runID uuid.UUID, reason string, actorID int64, idempotencyKey, action string) (*service.RunControlResult, error) {
	...
	case "cancel":
		...
		if _, err := tx.ExecContext(ctx, `UPDATE evaluation_runs SET status='cancelled', paused_from_status=NULL, pause_reason=NULL, cancelled_at=NOW(), cancelled_by=$2, finished_at=NOW(), control_epoch=$3, state_version=$4, updated_at=NOW() WHERE id=$1`, runID, actorID, newEpoch, newStateVersion); err != nil {

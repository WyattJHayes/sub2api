...
func TestEvaluationRepository_FreezesConfiguredRouteProfileAcrossRunAndSides(t *testing.T) {
	ctx := context.Background()
	fixture := createEvaluationRepositoryFixture(t, 1, []string{"route-a"}, 1)
	const configuredProfile = "main-gateway-v1"
	repo := NewEvaluationRepositoryWithRouteProfile(integrationDB, configuredProfile)
	...
	require.NoError(t, integrationDB.QueryRowContext(ctx,
		`SELECT route_profile_version FROM evaluation_runs WHERE id = $1`, run.ID).Scan(&runProfile))
	require.Equal(t, configuredProfile, runProfile)

	var mismatchedSides int
	require.NoError(t, integrationDB.QueryRowContext(ctx, `
		SELECT COUNT(*)
		FROM evaluation_side_specs side_spec
...
func TestEvaluationGradingRepository_FailedAssignmentTerminatesRun(t *testing.T) {
	...
	var assignmentStatus, sampleStatus, assignmentFailureClass, assignmentFailureCode string
	var sampleFailureClass, sampleFailureCode string
	require.NoError(t, integrationDB.QueryRowContext(ctx, `
		SELECT a.status, COALESCE(a.failure_class, ''), COALESCE(a.failure_code, ''),
		       s.status, COALESCE(s.failure_class, ''), COALESCE(s.failure_code, '')
		FROM evaluation_assignments a
		JOIN evaluation_samples s ON s.id = a.sample_id
		WHERE a.id = $1`, lease.ID).Scan(
		&assignmentStatus, &assignmentFailureClass, &assignmentFailureCode,
		&sampleStatus, &sampleFailureClass, &sampleFailureCode,
	))
	require.Equal(t, "infra_failed", assignmentStatus)
	require.Equal(t, "infra_failed", sampleStatus)
	require.Equal(t, "infrastructure", assignmentFailureClass)
	require.Equal(t, "upstream_rejected", assignmentFailureCode)
	require.Equal(t, "infrastructure", sampleFailureClass)
	require.Equal(t, "upstream_rejected", sampleFailureCode)

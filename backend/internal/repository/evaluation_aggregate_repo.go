...
func (r *evaluationGradingRepository) completeRevisionAnalysisJob(ctx context.Context, jobID uuid.UUID, leaseToken string, submission service.AggregateSubmission) (*service.AggregateSnapshot, error) {
	...
	err = tx.QueryRowContext(ctx, `
		SELECT job.run_id, run.tenant_id, job.capability_domain, job.model_route, job."window", job.analysis_version,
		...
		LEFT JOIN evaluation_workers w ON w.id=job.leased_by AND w.tenant_id=run.tenant_id
	...
	if submission.QualityReport != nil {
		if job.scope != "cell" {
			return nil, service.ErrInvalidQualityReportModelAlias
		}
		if err := insertQualityReportTx(ctx, tx, job.tenantID, job.runID, job.route, job.aggregateRevision, *submission.QualityReport); err != nil {
			return nil, err
		}
	}
	if _, err := tx.ExecContext(ctx, `

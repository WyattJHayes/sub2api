...
func radarCaseRows(input service.CreateRadarDatasetInput) ([]radarCaseRow, string, error) {
	...
	for _, item := range input.Cases {
		...
		qualityProbe := []byte(`{}`)
		if item.QualityDimension == nil && item.QualityProbeSpec != nil {
			return nil, "", fmt.Errorf("quality probe requires a dimension for %q", item.CaseKey)
		}
		if item.QualityDimension != nil {
			if item.QualityProbeSpec == nil || item.QualityProbeSpec.QualityDimension != *item.QualityDimension {
				return nil, "", fmt.Errorf("quality probe dimension mismatch for %q", item.CaseKey)
			}
			if err := item.QualityProbeSpec.Validate(); err != nil {
				return nil, "", fmt.Errorf("invalid quality probe for %q: %w", item.CaseKey, err)
			}
			qualityProbe, err = json.Marshal(item.QualityProbeSpec)
			if err != nil {
				return nil, "", fmt.Errorf("encode quality probe for %q: %w", item.CaseKey, err)
			}
		}
		digest := sha256.New()
		...
		rows = append(rows, radarCaseRow{input: item, prompt: prompt, expected: expected, execution: execution, qualityProbe: qualityProbe, contentSHA256: hex.EncodeToString(digest.Sum(nil))})
	}
	...
	for _, row := range rows {
		_, err = tx.ExecContext(ctx, `
			INSERT INTO evaluation_cases (
				...
				quality_dimension, quality_probe_spec
			) VALUES ($1, $2, $3, $4, $5, $6, $7, $8::jsonb, $9::jsonb, $10::jsonb,
				$11, $12, $13, $14, $15, $16, $17::jsonb)`, uuid.New(), record.ID, row.input.CaseKey,
			...
			row.input.EstimatedCost, row.input.QualityDimension, string(row.qualityProbe))
		if err != nil {
...
func (r *radarGovernanceRepository) CreateRunWithMatrix(ctx context.Context, input service.CreateRunInput) (*service.EvaluationRun, error) {
	return NewEvaluationRepositoryWithRouteProfile(r.db, r.routeProfileVersion).CreateRunWithMatrix(ctx, input)
}

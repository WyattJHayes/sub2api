...
func TestEvaluateReliabilitySufficiency(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "incomplete denominator", requirements: []ReliabilitySliceRequirement{latencyRequirement}, snapshots: []ReliabilitySnapshot{{ProfileID: "p1", SliceKey: "latency", FreshUntil: now.Add(time.Hour), Metrics: ReliabilityMetrics{RequestCount: 200, SuccessCount: 199, SuccessfulLatencyCount: 199}}}, wantStatus: ReliabilityInsufficientEvidence, wantReason: "reliability_denominator_incomplete"},
		{name: "billing not reconciled", requirements: []ReliabilitySliceRequirement{latencyRequirement}, snapshots: []ReliabilitySnapshot{{ProfileID: "p1", SliceKey: "latency", FreshUntil: now.Add(time.Hour), Metrics: ReliabilityMetrics{RequestCount: 200, SuccessCount: 200, SuccessfulLatencyCount: 200, BillingIdempotencyFailures: 1}}}, wantStatus: ReliabilityInsufficientEvidence, wantReason: "billing_not_reconciled"},
	}

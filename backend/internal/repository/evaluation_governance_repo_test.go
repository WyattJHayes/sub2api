...
func TestProvideRadarGovernanceRepositoryUsesConfiguredRouteProfile(t *testing.T) {
	const configuredProfile = "main-gateway-v1"
	repo := ProvideRadarGovernanceRepository(nil, &config.Config{Radar: config.RadarConfig{
		RouteProfileVersion: configuredProfile,
	}})

	configured, ok := repo.(*radarGovernanceRepository)
	require.True(t, ok)
...
func TestListModelHealthQualifiesAggregateSortColumns(t *testing.T) {
	db, mock, err := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	require.NoError(t, err)
	t.Cleanup(func() { _ = db.Close() })

	mock.ExpectQuery(`ORDER BY s\.model_route, s\.capability_domain, s\.window_start DESC, s\.created_at DESC`).
		WillReturnRows(sqlmock.NewRows([]string{
			"model_route", "capability_domain", "baseline_score", "candidate_score",
			"delta_pp", "ci_low_pp", "ci_high_pp", "effective_pair_count", "window_start",
		}))

	items, err := (&radarGovernanceRepository{db: db}).ListModelHealth(context.Background())
	require.NoError(t, err)
	require.Empty(t, items)
	require.NoError(t, mock.ExpectationsWereMet())
}

func TestListModelHealthProjectsReliabilityMetricsWithinTenant(t *testing.T) {
	...
	freshness := time.Date(2026, 8, 10, 12, 0, 0, 0, time.UTC)
	columns := []string{
	...
	mock.ExpectQuery(`(?s)WITH latest_aggregates AS .*route_metrics AS .*tracked_without_aggregate AS .*ORDER BY model_route, capability_domain`).
		WithArgs(int64(77)).
		WillReturnRows(sqlmock.NewRows(columns).
			AddRow("gpt-5.6-sol", "gpt-5.6-sol", "reasoning", "0.70", "0.75", "5", "1", "9", "42", "1200.5", "0.025", 2200.0, 0.5, freshness).
			AddRow("", "gpt-5.6-terra", "chat", "0.80", "0.80", "0", "-1", "1", "42", nil, nil, 950.0, 0.01, freshness))

	ctx := service.WithRadarTenant(context.Background(), 77)
	items, err := (&radarGovernanceRepository{db: db}).ListModelHealth(ctx)

	require.NoError(t, err)
	require.Len(t, items, 2)
	require.Equal(t, 1200.5, *items[0].P99MS)
	require.Equal(t, 0.025, *items[0].ErrorRate)
	require.Equal(t, 950.0, *items[1].P99MS)
	require.Equal(t, 0.01, *items[1].ErrorRate)
	require.Equal(t, 2, strings.Count(capturedQuery, "r.tenant_id=$1"))
	require.Contains(t, capturedQuery, "tm.tenant_id=$1")
	require.NoError(t, mock.ExpectationsWereMet())
}
...
func TestRegisterRadarWorkerIsIdentityIdempotent(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO evaluation_worker_events (id, worker_id, event_type, idempotency_key, actor_id, payload) VALUES ($1,$2,'registered',$3,$4,jsonb_build_object('token_fingerprint',$5::text,'worker_kind',$6::text,'name',$7::text))")).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()
...
func TestDrainWorkerCompletesAfterActiveLeaseCountZero(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("UPDATE evaluation_workers SET claim_mode=$1::text, drain_requested_at=CASE WHEN $1::text='draining' THEN COALESCE(drain_requested_at,NOW()) ELSE NULL END, updated_at=NOW() WHERE id=$2 AND status='active' RETURNING id, name, worker_kind, status, claim_mode, token_epoch, token_fingerprint, region, image_digest, capabilities, max_concurrency, created_at, updated_at")).WithArgs("draining", workerID).WillReturnRows(sqlmock.NewRows([]string{"id", "name", "worker_kind", "status", "claim_mode", "token_epoch", "token_fingerprint", "region", "image_digest", "capabilities", "max_concurrency", "created_at", "updated_at"}).AddRow(workerID, "runner-a", "runner", "active", "draining", int64(0), "abcd1234efgh", "cn-north", "sha256:runner", "{chat}", 1, time.Now(), time.Now()))
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO evaluation_worker_events (id, worker_id, event_type, idempotency_key, actor_id, payload) VALUES ($1,$2,$3,$4,$5,jsonb_build_object('claim_mode',$6::text))")).WillReturnResult(sqlmock.NewResult(1, 1))

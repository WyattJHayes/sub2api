	...
	for _, evaluationCase := range cases {
		...
		for matrixIndex, matrixEntry := range matrix {
			for sampleIndex := 0; sampleIndex < evaluationCase.sampleCount; sampleIndex++ {
				...
				baseline, err := makeEvaluationSideSpec("baseline", matrixEntry, routeProfileVersion)
				if err != nil {
					return result, err
				}
				candidate, err := makeEvaluationSideSpec("candidate", matrixEntry, routeProfileVersion)
				if err != nil {
					return result, err
				}
...
func makeEvaluationSideSpec(side string, entry evaluationMatrixEntry, routeProfileVersion string) (evaluationSidePersist, error) {
	...
	spec := service.SideSpec{
		...
		ExpectedResolvedModel: resolved, RouteProfileVersion: routeProfileVersion,

...
func NewEvaluationRepository(db *sql.DB) service.EvaluationRepository {
	return NewEvaluationRepositoryWithRouteProfile(db, radarRouteProfileVersion)
}

func NewEvaluationRepositoryWithRouteProfile(db *sql.DB, routeProfileVersion string) service.EvaluationRepository {
	return &evaluationRepository{db: db, routeProfileVersion: strings.TrimSpace(routeProfileVersion)}
}
...
func (r *evaluationRepository) CreateRunWithMatrix(ctx context.Context, input service.CreateRunInput) (*service.EvaluationRun, error) {
	...
	routeProfileVersion := strings.TrimSpace(r.routeProfileVersion)
	if routeProfileVersion == "" {
		return nil, errors.New("evaluation route profile version is required")
	}
	...
	err = tx.QueryRowContext(ctx, `
		...
		run.Status, run.BudgetLimit, run.ReservedCost, input.CreatedBy, routeProfileVersion).Scan(&run.CreatedAt, &runControlEpoch)
	if err != nil {
		return nil, fmt.Errorf("insert evaluation run: %w", err)
	}

	contracts, err := persistEvaluationExperimentContracts(ctx, tx, run.ID, run.CreatedAt, runControlEpoch, routeProfileVersion, cases, matrix)
	if err != nil {
		return nil, err
	}
...
func (r *evaluationRepository) TransitionAssignment(ctx context.Context, input service.AssignmentTransition) error {
	...
	if assignmentLeaseActive(input.To) {
		...
		assignmentQuery := `
			...
			SET status = $3, lease_token_hash = NULL, leased_by = NULL, lease_expires_at = NULL,
				heartbeat_at = NOW(), failure_class = NULLIF($5, ''), failure_code = NULLIF($6, ''),
			...
			WHERE a.id = $1 AND a.sample_id = s.id AND a.lease_token_hash = $2 AND a.lease_expires_at > NOW()
				...
				AND ($4::bigint IS NULL OR a.lease_epoch = $4)`
		assignmentArgs := []any{
			input.AssignmentID, hashToken(input.LeaseToken), input.To, expectedEpoch,
			strings.TrimSpace(string(input.FailureClass)), strings.TrimSpace(input.FailureCode),
		}
		if workerID, bound := service.RadarWorkerID(ctx); bound {
			assignmentQuery += ` AND a.leased_by = $7`
			assignmentArgs = append(assignmentArgs, workerID)
		}
		assignmentQuery += ` RETURNING a.sample_id`
		err = tx.QueryRowContext(ctx, assignmentQuery, assignmentArgs...).Scan(&sampleID)
	...
	if _, err := tx.ExecContext(ctx, `
		UPDATE evaluation_samples
		SET status = $2, failure_class = NULLIF($3, ''), failure_code = NULLIF($4, ''), updated_at = NOW()
		WHERE id = $1`, sampleID, input.To, strings.TrimSpace(string(input.FailureClass)), strings.TrimSpace(input.FailureCode)); err != nil {

...
type radarGovernanceRepository struct {
	db                  *sql.DB
	routeProfileVersion string
}

func NewRadarGovernanceRepository(db *sql.DB) service.RadarGovernanceRepository {
	return &radarGovernanceRepository{db: db, routeProfileVersion: radarRouteProfileVersion}
}
...
func projectionMetric(raw sql.NullString, fallback sql.NullFloat64) *float64 {
	if value := nullableProjectionFloat(raw); value != nil {
		return value
	}
	if !fallback.Valid {
		return nil
...
func (r *radarGovernanceRepository) ListModelHealth(ctx context.Context) ([]service.RadarModelHealthProjection, error) {
	...
	aggregateTenantFilter := ""
	routeTenantFilter := ""
	trackedTenantFilter := ""
	args := []any{}
	if tenantID, scoped := radarTenant(ctx); scoped {
		aggregateTenantFilter = ` AND r.tenant_id=$1`
		routeTenantFilter = ` AND r.tenant_id=$1`
		trackedTenantFilter = ` AND tm.tenant_id=$1`
		args = append(args, tenantID)
	}
	query := fmt.Sprintf(`
		WITH latest_aggregates AS (
			SELECT DISTINCT ON (s.model_route, s.capability_domain)
			       s.run_id, s.model_route, s.capability_domain, s.aggregate, s.window_start
			FROM evaluation_aggregate_snapshots s
			JOIN evaluation_runs r ON r.id=s.run_id
			WHERE 1=1%s
			ORDER BY s.model_route, s.capability_domain, s.window_start DESC, s.created_at DESC
		), route_metrics AS (
			SELECT e.evaluation_run_id AS run_id,
			       e.requested_model AS model_route,
			       COALESCE(c.capability_domain, '') AS capability_domain,
			       percentile_cont(0.99) WITHIN GROUP (ORDER BY e.latency_ms)
			           FILTER (WHERE e.latency_ms IS NOT NULL) AS p99_ms,
			       COUNT(*) FILTER (WHERE e.transport_status <> 'succeeded')::double precision
			           / NULLIF(COUNT(*), 0) AS error_rate
			FROM evaluation_route_evidence e
			JOIN evaluation_runs r ON r.id=e.evaluation_run_id
			LEFT JOIN evaluation_samples es ON es.id=e.sample_id
			LEFT JOIN evaluation_cases c ON c.id=es.case_id
			WHERE e.finished_at IS NOT NULL%s
			GROUP BY e.evaluation_run_id, e.requested_model, COALESCE(c.capability_domain, '')
		), tracked_models AS (
			SELECT DISTINCT tm.model_alias
			FROM evaluation_tracked_models tm
			WHERE 1=1%s
		), latest_with_metrics AS (
			SELECT COALESCE(tm.model_alias, '') AS model_alias,
			       la.model_route, la.capability_domain, la.aggregate, la.window_start,
			       rm.p99_ms AS route_p99_ms, rm.error_rate AS route_error_rate
			FROM latest_aggregates la
			LEFT JOIN route_metrics rm
			  ON rm.run_id=la.run_id
			 AND rm.model_route=la.model_route
			 AND rm.capability_domain=la.capability_domain
			LEFT JOIN tracked_models tm ON tm.model_alias=la.model_route
		), tracked_without_aggregate AS (
			SELECT tm.model_alias,
			       tm.model_alias AS model_route,
			       ''::text AS capability_domain,
			       NULL::jsonb AS aggregate,
			       tm.created_at AS window_start,
			       NULL::float8 AS route_p99_ms,
			       NULL::float8 AS route_error_rate
			FROM evaluation_tracked_models tm
			WHERE 1=1%s
			  AND NOT EXISTS (
				SELECT 1 FROM latest_aggregates la WHERE la.model_route=tm.model_alias
			  )
		), visible_models AS (
			SELECT model_alias, model_route, capability_domain, aggregate, window_start, route_p99_ms, route_error_rate
			FROM latest_with_metrics
			UNION ALL
			SELECT model_alias, model_route, capability_domain, aggregate, window_start, route_p99_ms, route_error_rate
			FROM tracked_without_aggregate
		)
		SELECT model_alias, model_route, capability_domain,
		       aggregate->>'baseline_score', aggregate->>'candidate_score',
		       aggregate->>'delta_pp', aggregate->>'ci_low_pp', aggregate->>'ci_high_pp',
		       aggregate->>'effective_pair_count', aggregate->>'p99_ms', aggregate->>'error_rate',
		       route_p99_ms, route_error_rate, window_start
		FROM visible_models
		ORDER BY model_route, capability_domain`, aggregateTenantFilter, routeTenantFilter, trackedTenantFilter, trackedTenantFilter)
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
	...
	for rows.Next() {
		var modelAlias, model, domain string
		var baseline, candidate, delta, low, high, countRaw, aggregateP99, aggregateErrorRate sql.NullString
		var routeP99, routeErrorRate sql.NullFloat64
		var freshness time.Time
		if err := rows.Scan(&modelAlias, &model, &domain, &baseline, &candidate, &delta, &low, &high, &countRaw, &aggregateP99, &aggregateErrorRate, &routeP99, &routeErrorRate, &freshness); err != nil {
		...
		out = append(out, service.RadarModelHealthProjection{
			ModelAlias: modelAlias, ModelRoute: model, CapabilityDomain: domain,
			...
			SampleCount: func() *int {
				...
				return nil
			}(), P99MS: projectionMetric(aggregateP99, routeP99), ErrorRate: projectionMetric(aggregateErrorRate, routeErrorRate), Freshness: freshness,
		})
	}
	return out, rows.Err()
}

func (r *radarGovernanceRepository) RegisterTrackedModel(ctx context.Context, modelAlias string, actorID int64) (*service.RadarTrackedModel, error) {
	if err := r.valid(); err != nil {
		return nil, err
	}
	modelAlias = strings.TrimSpace(modelAlias)
	if modelAlias == "" || actorID <= 0 {
		return nil, errors.New("model alias and actor are required")
	}
	tenantID := actorID
	if scopedTenantID, scoped := radarTenant(ctx); scoped {
		tenantID = scopedTenantID
	}

	tx, err := beginRadarWriterTx(ctx, r.db, "api")
	if err != nil {
		return nil, fmt.Errorf("begin tracked model registration: %w", err)
	}
	defer func() { _ = tx.Rollback() }()

	model := &service.RadarTrackedModel{}
	err = tx.QueryRowContext(ctx, `
		INSERT INTO evaluation_tracked_models (tenant_id, model_alias, created_by)
		VALUES ($1, $2, $3)
		ON CONFLICT (tenant_id, model_alias) DO NOTHING
		RETURNING model_alias, created_by, created_at`, tenantID, modelAlias, actorID).Scan(
		&model.ModelAlias, &model.CreatedBy, &model.CreatedAt,
	)
	if errors.Is(err, sql.ErrNoRows) {
		err = tx.QueryRowContext(ctx, `
			SELECT model_alias, created_by, created_at
			FROM evaluation_tracked_models
			WHERE tenant_id=$1 AND model_alias=$2`, tenantID, modelAlias).Scan(
			&model.ModelAlias, &model.CreatedBy, &model.CreatedAt,
		)
	}
	if err != nil {
		return nil, fmt.Errorf("register tracked model: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO quality_policy_versions (id, tenant_id, version, policy, created_by)
		VALUES ($1, $2, 'quality-v1', $3::jsonb, $4)
		ON CONFLICT (tenant_id, version) DO NOTHING`, uuid.New(), tenantID, service.DefaultQualityPolicyJSON(), actorID); err != nil {
		return nil, fmt.Errorf("create default quality policy: %w", err)
	}
	if err := tx.Commit(); err != nil {
		return nil, fmt.Errorf("commit tracked model registration: %w", err)
	}
	return model, nil
}

func (r *radarGovernanceRepository) UntrackModel(ctx context.Context, modelAlias string, actorID int64) error {
	if err := r.valid(); err != nil {
		return err
	}
	...
	if actorID <= 0 {
		return errors.New("tracked model actor is required")
	}
	modelAlias = strings.TrimSpace(modelAlias)
	if modelAlias == "" {
		return service.ErrTrackedModelNotFound
	}
	result, err := r.db.ExecContext(ctx, `
		DELETE FROM evaluation_tracked_models
		WHERE tenant_id=$1 AND model_alias=$2`, tenantID, modelAlias)
	if err != nil {
		return fmt.Errorf("untrack radar model: %w", err)
	}
	affected, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("read untrack radar model result: %w", err)
	}
	if affected == 0 {
		return service.ErrTrackedModelNotFound
	}
...
func (r *radarGovernanceRepository) RegisterRadarWorker(ctx context.Context, input service.RadarWorkerRegistrationInput, actorID int64, idempotencyKey string) (*service.RadarWorkerRecord, error) {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO evaluation_worker_events (id, worker_id, event_type, idempotency_key, actor_id, payload) VALUES ($1,$2,'registered',$3,$4,jsonb_build_object('token_fingerprint',$5::text,'worker_kind',$6::text,'name',$7::text))`, uuid.New(), worker.ID, idempotencyKey, actorID, worker.TokenFingerprint, worker.WorkerKind, worker.Name); err != nil {
...
func (r *radarGovernanceRepository) RotateRadarWorkerToken(ctx context.Context, workerID uuid.UUID, token string, actorID int64, idempotencyKey string) (*service.RadarWorkerRecord, error) {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO evaluation_worker_events (id, worker_id, event_type, idempotency_key, actor_id, payload) VALUES ($1,$2,'token_rotated',$3,$4,jsonb_build_object('token_fingerprint',$5::text,'token_epoch',$6::bigint))`, uuid.New(), workerID, idempotencyKey, actorID, worker.TokenFingerprint, worker.TokenEpoch); err != nil {
		return nil, err
	}
	if err := tx.Commit(); err != nil {
		return nil, err
...
func (r *radarGovernanceRepository) SetRadarWorkerClaimMode(ctx context.Context, workerID uuid.UUID, mode service.WorkerClaimMode, actorID int64, idempotencyKey string) (*service.RadarWorkerRecord, error) {
	...
	worker, err := scanRadarWorker(tx.QueryRowContext(ctx, `UPDATE evaluation_workers SET claim_mode=$1::text, drain_requested_at=CASE WHEN $1::text='draining' THEN COALESCE(drain_requested_at,NOW()) ELSE NULL END, updated_at=NOW() WHERE id=$2 AND status='active' RETURNING id, name, worker_kind, status, claim_mode, token_epoch, token_fingerprint, region, image_digest, capabilities, max_concurrency, created_at, updated_at`, mode, workerID))
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, infraerrors.New(http.StatusConflict, "WORKER_UNAVAILABLE", "worker is unavailable")
		}
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO evaluation_worker_events (id, worker_id, event_type, idempotency_key, actor_id, payload) VALUES ($1,$2,$3,$4,$5,jsonb_build_object('claim_mode',$6::text))`, uuid.New(), workerID, eventType, idempotencyKey, actorID, mode); err != nil {
		return nil, err
	}
...
func (r *radarGovernanceRepository) DisableRadarWorker(ctx context.Context, workerID uuid.UUID, actorID int64, idempotencyKey string) (*service.RadarWorkerRecord, error) {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO evaluation_worker_events (id, worker_id, event_type, idempotency_key, actor_id, payload) VALUES ($1,$2,'disabled',$3,$4,jsonb_build_object('token_fingerprint',$5::text))`, uuid.New(), workerID, idempotencyKey, actorID, worker.TokenFingerprint); err != nil {
		return nil, err
	}
	if err := tx.Commit(); err != nil {
		return nil, err
...
func (r *radarGovernanceRepository) ListDatasets(ctx context.Context) ([]service.RadarDatasetProjection, error) {
	...
	query := `
		SELECT d.id, d.dataset_key, d.version, d.status, COUNT(c.id), d.source_type,
			d.created_by, d.tenant_id, d.created_at
	...
	query += `
		GROUP BY d.id, d.dataset_key, d.version, d.status, d.source_type, d.created_by, d.tenant_id, d.created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Name, &item.Version, &item.Status, &item.Cases, &item.SourceType, &item.CreatedBy, &item.TenantID, &item.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, item)
	}

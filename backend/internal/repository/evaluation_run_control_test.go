...
func TestCancelPausedRunClearsPauseFields(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	runID := uuid.New()
	expectRadarWorkerWriter(t, mock)
	mock.ExpectQuery("SELECT id, run_id, event_type, payload").WithArgs(strings.Repeat("f", 64)).WillReturnError(sql.ErrNoRows)
	mock.ExpectQuery("SELECT status, paused_from_status, pause_reason, control_epoch, state_version").WithArgs(runID).
		WillReturnRows(sqlmock.NewRows([]string{"status", "paused_from_status", "pause_reason", "control_epoch", "state_version"}).AddRow("paused", "running", "operator", int64(4), int64(7)))
	mock.ExpectExec("UPDATE evaluation_assignments SET status='cancelled'").WithArgs(runID).WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("UPDATE evaluation_samples SET status='cancelled'").WithArgs(runID).WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("UPDATE evaluation_grading_jobs SET status='cancelled'").WithArgs(runID).WillReturnResult(sqlmock.NewResult(0, 6))
	mock.ExpectExec("UPDATE evaluation_analysis_jobs SET status='cancelled'").WithArgs(runID).WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("INSERT INTO evaluation_route_evidence_terminalization_outbox").WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec("UPDATE evaluation_runs SET status='cancelled', paused_from_status=NULL, pause_reason=NULL").
		WithArgs(runID, int64(9), int64(5), int64(8)).WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec("INSERT INTO evaluation_run_events").WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()

	repo := &radarGovernanceRepository{db: db}
	result, err := repo.CancelRun(context.Background(), runID, "recovery", 9, strings.Repeat("f", 64))
	require.NoError(t, err)
	require.Equal(t, "paused", result.FromStatus)
	require.Equal(t, "cancelled", result.ToStatus)
	require.Equal(t, int64(5), result.CurrentEpoch)
	require.NoError(t, mock.ExpectationsWereMet())
}

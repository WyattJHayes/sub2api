...
const (
	...
	radarP0RouteProfile      = "main-gateway-v1"
...
func provisionRadarP0EvaluationLease(t *testing.T, db *sql.DB, userID, evaluationKeyID int64) *service.AssignmentLease {
	...
	require.NoError(t, err)
	run, err := repository.NewEvaluationRepositoryWithRouteProfile(db, radarP0RouteProfile).CreateRunWithMatrix(ctx, service.CreateRunInput{
